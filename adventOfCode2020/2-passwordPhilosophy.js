...
const ex =   '15-19 k: kkkkkkkkkkkkzkkkkkkk'

  const validate = arr => {
    const isValid = pwd => {
      const low = parseInt(pwd.slice(0, pwd.indexOf('-')))
      const high = parseInt(pwd.slice(pwd.indexOf('-') + 1, pwd.indexOf(' ')))
      const char = pwd.slice(pwd.indexOf(' ') + 1, pwd.indexOf(':'))

      let count = 0
      for (let c of pwd.slice(pwd.indexOf(': ') + 2)) {
        if (c === char) count++;
